<?php

namespace Database\Seeders;

use DateTime;
use Illuminate\Database\Seeder;
use Illuminate\Support\Facades\DB;

class VideoSeeder extends Seeder
{
    /**
     * Run the database seeds.
     */
    public function run(): void
    {
        DB::table("videos")->insert([
            [
                "url" => "Ij1o5zGDCeE",
                "question_id" => 1,
                "start_time" => 215,
                "answer" => "We don't just live as individuals, we live embedded in a society",
                "end_time" => 220,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "2Oe6HUgrRlQ",
                "question_id" => 2,
                "start_time" => 323,
                "answer" => "who went up and down this country delivering sermons,",
                "end_time" => 326,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "o1Z4F4e2Bw4",
                "question_id" => 3,
                "start_time" => 375,
                "answer" => "a place that had been barely visited by paleontologists.",
                "end_time" => 379,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "22aeMgJ5cWY",
                "question_id" => 4,
                "start_time" => 313,
                "answer" => 'As late as 1952 when Arthur Miller visited to research "The Crucible", the subject of witchcraft was taboo.',
                "end_time" => 320,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "Ij1o5zGDCeE",
                "question_id" => 5,
                "start_time" => 70,
                "answer" => 'and often commits great harm in the present in order to get there.',
                "end_time" => 74,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "2Oe6HUgrRlQ",
                "question_id" => 6,
                "start_time" => 656,
                "answer" => "That's why a very common feeling when you're in a museum --",
                "end_time" => 658,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "Ij1o5zGDCeE",
                "question_id" => 7,
                "start_time" => 90,
                "answer" => "the idea that we are going to go back to some imaginary past",
                "end_time" => 93,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "C-GrRIgdmW8",
                "question_id" => 8,
                "start_time" => 4,
                "answer" => "I played this chord that was so wrong I thought I had just",
                "end_time" => 13,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "DjcZrtcBZi4",
                "question_id" => 9,
                "start_time" => 804,
                "answer" => "I don't know if you've ever looked at the credits of a Pixar movie,",
                "end_time" => 807,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "JW8dqT90BLs",
                "question_id" => 10,
                "start_time" => 33,
                "answer" => "us to peer at the primary building blocks of matter--",
                "end_time" => 36,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "2Oe6HUgrRlQ",
                "question_id" => 11,
                "start_time" => 250,
                "answer" => "that we might previously have found in the Gospel of Saint John.",
                "end_time" => 253,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "8i-pNVj5KMw",
                "question_id" => 12,
                "start_time" => 457,
                "answer" => "They listened to him, prayed with him, helped him question himself.",
                "end_time" => 464,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "uzKBGtf0i0M",
                "question_id" => 13,
                "start_time" => 18,
                "answer" => "When I first heard this theme,",
                "end_time" => 20,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "8dKFDr4pFWw",
                "question_id" => 14,
                "start_time" => 764,
                "answer" => "You had to take everyone out of Battery Park.",
                "end_time" => 766,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "8dKFDr4pFWw",
                "question_id" => 15,
                "start_time" => 249,
                "answer" => "That is very different from a traditional bank card where 75% of their revenues are",
                "end_time" => 255,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "7n9IOH0NvyY",
                "question_id" => 16,
                "start_time" => 784,
                "answer" => "less likely to be put on medication, more likely to go to the gym,",
                "end_time" => 788,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "aOPPpsgdPdI",
                "question_id" => 17,
                "start_time" => 53,
                "answer" => "The gas station has definitely become",
                "end_time" => 57,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "V5jjIHaKd-8",
                "question_id" => 18,
                "start_time" => 84,
                "answer" => "For me, the zoo was a way to get to know my camera",
                "end_time" => 88,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "F8Rwz3KWFHA",
                "question_id" => 19,
                "start_time" => 301,
                "answer" => "We've also discussed the threat from Islamist terrorism.",
                "end_time" => 304,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "aOPPpsgdPdI",
                "question_id" => 20,
                "start_time" => 203,
                "answer" => "In the grand scheme of things, no matter what celebrity is wearing",
                "end_time" => 207,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "JW8dqT90BLs",
                "question_id" => 21,
                "start_time" => 33,
                "answer" => "us to peer at the primary building blocks of matter--",
                "end_time" => 36,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "F8Rwz3KWFHA",
                "question_id" => 10,
                "start_time" => 242,
                "answer" => "First, we have talked about Britain's decision to leave",
                "end_time" => 245,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "oKtALHe3Y9Q",
                "question_id" => 22,
                "start_time" => 168,
                "answer" => "the first step to solving any problem",
                "end_time" => 170,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "3OgsRa7VDtI",
                "question_id" => 23,
                "start_time" => 243,
                "answer" => "are over budget or behind schedule.",
                "end_time" => 245,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "DjcZrtcBZi4",
                "question_id" => 24,
                "start_time" => 413,
                "answer" => "where you act, as opposed to plan, your way to the future.",
                "end_time" => 417,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
            [
                "url" => "etQJ1FUbOxk",
                "question_id" => 25,
                "start_time" => 148,
                "answer" => "work is because I talk to so many",
                "end_time" => 150,
                "created_at" => new DateTime(),
                "updated_at" => new DateTime(),
            ],
        ]);
    }
}
